 ...
 * @returns {Promise}
 */
export function onBeforeCalculate(quoteModel, quoteLineModel, conn) {
	console.log('onBeforeCalculate Function');
	var accountId = quoteModel.record['SBQQ__Account__c'];		
	console.log('Query >>>>>>>> 0'+ accountId);	
	var query = "SELECT id, Name FROM Account WHERE id = \'"+accountId+"\'";		
	console.log('Query >>>>>>>> 1'+ query);
	
	return conn.query(query).then(function(results) {		
		console.log('Query >>>>>>>> 2'+ JSON.stringify(results));		  
		if(results.totalSize){
			var account = results.records[0];
			if(account.Name.toUpperCase().indexOf('TEST')){
				quoteLineModel.forEach(function(line) {
					console.log('It >>>>>>>>>>>');
					line.record['SBQQ__SpecialPrice__c'] = 0;
					line.record['SBQQ__SpecialPriceType__c'] = 'Custom';
					line.record['SBQQ__SpecialPriceDescription__c'] = 'Testing QLI';
				});
			}
			
		}
		console.log('onBeforeCalculate Exit');
...
export function onBeforePriceRules(quoteModel, quoteLineModels, conn) {
	console.log('onBeforePriceRules Function');
    return Promise.resolve();
};
 ...
 * @returns {Promise}
 */
export function onAfterPriceRules(quote, quoteLine, conn) {
	console.log('onAfterPriceRules Function');	
    return Promise.resolve();
};
...
export function onAfterCalculate(quoteModel, quoteLineModels, conn) {
	console.log('onAfterCalculate Function');
    return Promise.resolve();
};